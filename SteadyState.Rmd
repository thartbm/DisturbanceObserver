...
} else {remotes::install_github('thartbm/Reach')}
```
...
We run an ANOVA on the steady state levels:
...
In the contrasts (after correcting for multiple corrections) we now see there is only an effect of instructions in the lowest level of alpha (first four lines of the table).

The both in ignore (lines 5-7) and learn (lines 8-10) there is a difference between alpha 0.6 and 0.8, but no other effects. This is not great.
